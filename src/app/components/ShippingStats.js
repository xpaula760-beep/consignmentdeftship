...
export default function ShippingStats() {
  ...
  useEffect(() => {
    // Generate slight variations on refresh to make the data feel "live"
    setStats({
      savingsPercent: 89 + (Math.floor(Math.random() * 3) - 1), // 88, 89, or 90
      merchants: 3328 + Math.floor(Math.random() * 50),       // Adds 0-50 more merchants
      totalSavings: 18509633 + Math.floor(Math.random() * 5000), // Adds random dollar amount
    });
  }, []);
